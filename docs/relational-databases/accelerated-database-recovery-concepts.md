...
- **Multi-threaded version cleanup**  
  ...
  To adjust the number of threads for version cleanup scalability, set `ADR Cleaner Thread Count` with `sp_configure`. The numer of threads is capped at the number of cores allocated to the instance.
